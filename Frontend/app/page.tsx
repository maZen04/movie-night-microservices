...
import { addToWatchlist, answerRecommendation, api, authenticate, completeRecommendation, getMovieDetails, getWatchedHistory, getWatchlist, logout, markMovieWatched, removeFromWatched, removeFromWatchlist, searchMovies, sessionSocket, startRecommendation, type BackendMovie, type RecommendationQuestion } from '@/lib/api'
import {
  ArrowLeft,
  Bot,
  Check,
...
const participants = [
  { name: 'Maya Chen', initials: 'MC', color: 'bg-accent' },
  { name: 'Jordan Lee', initials: 'JL', color: 'bg-[#7a6d92]' },
  { name: 'Sam Rivera', initials: 'SR', color: 'bg-[#5e7c78]' },
]
...
export default function Page() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      ...
      {view === 'waiting' && <Waiting onStart={() => setView('voting')} onLeave={() => setView('home')} />}
      {view === 'voting' && <Voting movie={movies[voteIndex % movies.length]} index={voteIndex} onVote={() => setVoteIndex((i) => i + 1)} onFinish={() => setView('result')} />}
      {view === 'result' && <Result movie={movies[3]} onHome={() => setView('home')} />}
      {(view === 'login' || view === 'register') && <Auth register={view === 'register'} onSwitch={() => setView(view === 'login' ? 'register' : 'login')} onEnter={() => setView('home')} />}

      {sessionModal && <SessionModal mode={sessionModal} onClose={() => setSessionModal(null)} onDone={() => { setSessionModal(null); setView('waiting') }} />}
...
function Waiting({ onStart, onLeave }: { onStart: () => void; onLeave: () => void }) { return <section className="mx-auto flex min-h-[calc(100vh-80px)] max-w-3xl flex-col justify-center px-5 pb-20 sm:px-8"><button onClick={onLeave} className="mb-16 flex items-center gap-2 self-start text-sm text-muted-foreground hover:text-foreground"><X className="size-4" /> Leave session</button><div className="flex items-start justify-between gap-5"><div><p className="mb-3 text-xs uppercase tracking-[0.24em] text-accent">Movie night session</p><h1 className="font-serif text-5xl tracking-tight sm:text-6xl">Friday at Maya&apos;s</h1><p className="mt-4 text-sm text-muted-foreground">Everyone&apos;s here. Almost time to choose.</p></div><div className="rounded-md border border-border bg-card px-4 py-3 text-center"><p className="text-[10px] uppercase tracking-widest text-muted-foreground">Code</p><p className="mt-1 font-mono text-lg tracking-[0.2em]">M8K2Q</p></div></div><div className="mt-14 rounded-lg border border-border bg-card/70 p-5 sm:p-7"><div className="flex items-center justify-between border-b border-border pb-5"><span className="flex items-center gap-2 text-sm font-medium"><Users className="size-4 text-accent" /> 3 participants</span><span className="flex items-center gap-2 text-xs text-accent"><span className="size-2 animate-pulse rounded-full bg-accent" /> Ready when you are</span></div><div className="flex flex-col gap-4 py-6">{participants.map((person, index) => <div key={person.name} className="flex items-center justify-between"><div className="flex items-center gap-3"><span className={`flex size-10 items-center justify-center rounded-full text-xs font-semibold text-foreground ${person.color}`}>{person.initials}</span><span className="text-sm">{person.name}</span>{index === 0 && <span className="rounded-full border border-accent/30 px-2 py-1 text-[10px] uppercase tracking-wider text-accent">Leader</span>}</div><span className="text-xs text-muted-foreground">Joined</span></div>)}</div>{<button onClick={onStart} className="h-13 w-full rounded-md bg-accent text-sm font-semibold uppercase tracking-[0.16em] text-accent-foreground transition hover:brightness-110">Start session</button>}</div><p className="mt-6 text-center text-xs text-muted-foreground">Share the code with friends to invite them.</p></section> }

function Voting({ movie, index, onVote, onFinish }: { movie: typeof movies[number]; index: number; onVote: () => void; onFinish: () => void }) { return <section className="mx-auto flex min-h-[calc(100vh-80px)] max-w-3xl flex-col items-center px-5 pb-16 pt-4 text-center sm:px-8"><div className="mb-8 flex w-full items-center justify-between text-xs text-muted-foreground"><span className="flex items-center gap-2"><span className="size-2 animate-pulse rounded-full bg-accent" /> Live session</span><span>{Math.min(index + 1, 8)} <span className="text-border">/</span> 8</span></div><p className="text-xs uppercase tracking-[0.24em] text-accent">Would you watch this?</p><div className="relative mt-5 aspect-[2/3] w-[min(68vw,330px)] overflow-hidden rounded-xl shadow-2xl shadow-black/40"><img key={movie.title} src={movie.image} alt={`${movie.title} poster`} className="h-full w-full object-cover" /><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 to-transparent p-5 pt-20 text-left"><h1 className="font-serif text-3xl text-white">{movie.title}</h1><p className="mt-1 text-xs text-white/70">{movie.year} · {movie.genre}</p></div></div><div className="mt-6 flex items-center gap-5"><button onClick={onVote} className="flex size-14 items-center justify-center rounded-full border border-border bg-card text-muted-foreground transition hover:border-destructive hover:text-destructive" aria-label="Dislike"><ThumbsDown className="size-5" /></button><button onClick={onVote} className="flex size-[4.5rem] items-center justify-center rounded-full bg-accent text-accent-foreground shadow-lg shadow-accent/15 transition hover:scale-105" aria-label="Like"><ThumbsUp className="size-6" /></button></div><p className="mt-5 max-w-sm text-xs leading-5 text-muted-foreground">{movie.overview}</p><button onClick={onFinish} className="mt-8 text-xs text-muted-foreground underline underline-offset-4 hover:text-foreground">See tonight&apos;s result</button></section> }

function Result({ movie, onHome }: { movie: typeof movies[number]; onHome: () => void }) { return <section className="relative mx-auto flex min-h-[calc(100vh-80px)] max-w-5xl flex-col items-center justify-center overflow-hidden px-5 pb-20 text-center sm:px-8"><div className="pointer-events-none absolute inset-x-0 top-0 h-96 opacity-30"><img src={movie.backdrop} alt="" className="h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-b from-background/20 to-background" /></div><div className="relative"><p className="mb-5 text-xs uppercase tracking-[0.28em] text-accent">Decision made</p><h1 className="font-serif text-5xl tracking-tight sm:text-7xl">Tonight&apos;s movie.</h1><img src={movie.image} alt={`${movie.title} poster`} className="mx-auto mt-10 w-48 rounded-lg shadow-2xl sm:w-56" /><h2 className="mt-7 font-serif text-3xl">{movie.title}</h2><div className="mt-3 flex items-center justify-center gap-4 text-xs text-muted-foreground"><span className="flex items-center gap-1 text-accent"><Star className="size-3 fill-accent" /> {movie.rating}</span><span>{movie.year}</span></div><p className="mx-auto mt-5 max-w-md text-sm leading-6 text-muted-foreground">{movie.overview}</p><button onClick={onHome} className="mt-8 h-12 rounded-md bg-accent px-6 text-sm font-semibold text-accent-foreground">Back home</button></div></section> }

function SessionModal({ mode, onClose, onDone }: { mode: 'create' | 'join'; onClose: () => void; onDone: () => void }) { const [name, setName] = useState(''); const [error, setError] = useState(''); const [loading, setLoading] = useState(false); async function submit() { if (!name.trim()) return; setLoading(true); setError(''); try { if (mode === 'create') await api('/api/sessions', { method: 'POST', body: JSON.stringify({ name: name.trim() }) }); else await api('/api/sessions/join', { method: 'POST', body: JSON.stringify({ code: name.trim() }) }); onDone() } catch (e) { setError(e instanceof Error ? e.message : 'Could not connect to the session.') } finally { setLoading(false) } } return <div className="fixed inset-0 z-40 flex items-center justify-center bg-background/80 px-5 backdrop-blur-sm"><div className="w-full max-w-md rounded-xl border border-border bg-card p-6 shadow-2xl sm:p-8"><div className="flex items-start justify-between"><div><p className="text-xs uppercase tracking-[0.2em] text-accent">{mode === 'create' ? 'Start something' : 'You’re invited'}</p><h2 className="mt-3 font-serif text-3xl">{mode === 'create' ? 'Create a session' : 'Join a session'}</h2></div><button onClick={onClose} className="rounded-full p-2 text-muted-foreground hover:text-foreground" aria-label="Close"><X className="size-4" /></button></div><label className="mt-8 block text-xs font-medium text-muted-foreground">{mode === 'create' ? 'Session name' : 'Session code'}</label><input value={name} onChange={(e) => setName(e.target.value)} placeholder={mode === 'create' ? 'Friday at Maya’s' : 'M8K2Q'} className="mt-2 h-12 w-full rounded-md border border-border bg-background px-4 font-mono text-sm outline-none focus:border-accent" />{error && <p className="mt-3 text-sm text-destructive">{error}</p>}<button onClick={submit} disabled={loading} className="mt-5 h-12 w-full rounded-md bg-accent text-sm font-semibold uppercase tracking-[0.14em] text-accent-foreground disabled:opacity-60">{loading ? 'Connecting…' : mode === 'create' ? 'Create session' : 'Join session'}</button></div></div> }
